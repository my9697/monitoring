...
export class Tracker {
  ...
  public constructor(appId: string, options?: Options) {
    ...
    ;(window as any).userId = this.setUserId.bind(this)
    ;(window as any).extra = this.setExtra.bind(this)
    this.installInnerTracker()
  }

  private installInnerTracker() {
    if (this.options.performanceTracker) {
      this.performanceTracker = new PerformanceTracker(this.options.performanceTracker, this.report.bind(this))
    }
    if (this.options.userActionTracker) {
      this.userActionTracker = new UserActionTracker(this.options.userActionTracker, this.report.bind(this))
    }
